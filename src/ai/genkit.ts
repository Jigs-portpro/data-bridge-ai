...
if (process.env.GOOGLEAI_API_KEY) {
  try {
    ...
    console.log("Google AI plugin loaded (using GOOGLEAI_API_KEY).");
  } catch (e) {
    console.warn("Failed to import or initialize Google AI plugin. GOOGLEAI_API_KEY might be set, but the plugin couldn't load. Error:", e);
  }
} else {
  console.log("GOOGLEAI_API_KEY not set. Google AI plugin will not be loaded.");
}
