...
if (anthropicKey) {
  console.log("ANTHROPIC_API_KEY found in environment. Attempting to load Anthropic plugin (but it was removed from package.json due to install issues)...");
  // try {
  //   const { anthropic } = await import('@google-cloud/genkit-anthropic');
  ...
  // }
  console.warn("Anthropic plugin was removed from package.json due to persistent npm install errors. Anthropic features will be unavailable.");
} else {
...
if (plugins.length === 0) {
  console.error(
    ...
    "or the Anthropic plugin was removed. AI features will likely fail. Please set at least one API key."
  );
}
